...
def download_cover(
    ...
    _path: str,
    timeout: int = 10,
    should_resize_to_square: bool = True,
) -> str:
    ...
    if "covers" in media_info:
        try:
            best = next(
                iter(
                    sorted(
                        media_info["covers"].items(),
                        key=lambda i: i[1]["width"],
                        reverse=True,
                    )
                ),
                None,
            ...
            if best:
                ...
                with open(downloaded_cover_path, "wb") as w:
                    if not should_resize_to_square:
                        ...
                        resize_url = (
                            f"https://ic.od-cdn.com/resize?type=auto"
        ...
        if "cover510Wide" in media_info["covers"]:
            ...
            with open(downloaded_cover_path, "wb") as w:
                if not should_resize_to_square:
                    w.write(
                        requests.get(
                            media_info["covers"]["cover510Wide"]["href"],
                            timeout=timeout,
                        ).content
                    )
                else:
...
def get_media_info(title_id: str, timeout: int = 10) -> dict:
    ...
    return requests.get(
        f"https://thunder.api.overdrive.com/v2/media/{title_id}", timeout=timeout
    ).json()


def is_book_available(library: str, title_id: str, timeout: int = 10) -> bool:
    availability = requests.get(
        f"https://thunder.api.overdrive.com/v2/libraries/{library}/media/{title_id}/availability",
        timeout=timeout,
    ).json()
...
def get_authors(media_info: dict, delim=" & ") -> str:
    return delim.join(
        [
            creator["name"]
            for creator in media_info["creators"]
...
def get_download_path(
    media_info: dict, format_string="%a/%y - %t", should_replace_space=False
) -> str:
    ...
    if "publishDate" in media_info:
        format_string = format_string.replace(
            "%y", str(compat_datetime_fromisoformat(media_info["publishDate"]).year)
        )
    else:
...
def get_toc_from_audiobook_info(audiobook_info: dict) -> dict:
    ...
    def get_marker(entry: dict) -> str:
        ...
        time = (
            convert_seconds_to_timestamp(timestamp_temp[-1])
            if len(timestamp_temp) != 1
            else "0:00.000"
        )
        new_entry = {
            "Name": "(continued)"
            if ("(00:00)" in entry["title"] and time == "0:00.000")
            else entry["title"],
            "Time": time,
        }
    ...
    for key, value in toc.items():
        tocout[key] = dicttoxml.dicttoxml(
            value,
            custom_root="Markers",
...
<ns0:metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">"""
    ...
    if "description" in media_info:
        description = re.sub(
            "<.*?>",
            "",
            media_info["description"].replace("<br>", "\n").replace("<BR>", "\n"),
        )
...
def embed_tag_data(
    filename: str, toc_entry_for_file: str, audiobook_info: dict, cover_file_path: str
):
    ...
    if "publishDate" in audiobook_info["media_info"]:
        year = TYER(
            text=str(
                compat_datetime_fromisoformat(
                    audiobook_info["media_info"]["publishDate"]
                ).year
            )
        )
        year2 = TDRL(
            text=compat_datetime_fromisoformat(
                audiobook_info["media_info"]["publishDate"]
            ).strftime("%Y-%m-%d")
        )
        tag.add(year)
        tag.add(year2)
    narrator = TCOM(
        text=get_narrators(media_info=audiobook_info["media_info"], delim="/")
    )
    tag.add(narrator)
    desc = COMM(
        lang="\x00\x00\x00",
        ...
        text=re.sub(
            "<\\/?[BIbiPp]>",
            "",
            html.unescape(audiobook_info["media_info"]["description"])
            .replace("<br>", "\n")
            .replace("<BR>", "\n"),
        ),
    )
    ...
    genre = TCON(
        text=";".join(
            map(lambda x: x["name"], audiobook_info["media_info"]["subjects"])
        )
    )
    tag.add(genre)
    # IF NOT SERIES
    if "detailedSeries" in audiobook_info["media_info"]:
        series = TXXX(
            desc="MVNM",
            text=audiobook_info["media_info"]["detailedSeries"]["seriesName"],
        )
        tag.add(series)
        if "readingOrder" in audiobook_info["media_info"]["detailedSeries"]:
            vol_number = TXXX(
                desc="MVIN",
                text=audiobook_info["media_info"]["detailedSeries"]["readingOrder"],
            )
            tag.add(vol_number)
    ...
    overdrive_mediamarkers = TXXX(
        desc="OverDrive MediaMarkers", text=toc_entry_for_file
    )
    ...
    chapters = sorted(
        [
            (m[0].text, convert_timestamp_to_seconds(m[1].text))
            for m in ET.fromstring(toc_entry_for_file).findall("Marker")
        ],
        key=lambda x: x[1],
    )
    chapter_tags = []
    ...
    for i in range(len(chapters)):
        c = CHAP(
            element_id=f"Chapter {i}",
            start_time=start_time,
            end_time=chapters[i + 1][1] * 1000
            if len(chapters) > i + 1
            else int(file.info.length * 1000),
            sub_frames=[TIT2(text=chapters[i][0])],
        )
...
class Libby:
    ...
    def __init__(
        ...
        if os.path.isfile(id_path):
            ...
            self.http_session.headers.update(
                {"Authorization": f'Bearer {identity["identity"]}'}
            )
            if not self.is_logged_in():
                if code:
                    ...
                    if not self.is_logged_in():
                        raise RuntimeError(
                            "Couldn't log in with code, are you sure you wrote it correctly and that "
            ...
            raise RuntimeError(
                "PyLibby needs a path to a JSON-file with ID-info. To get the file you"
    ...
    def borrow_book(self, title_id: str, card_id: str, days: int = 21) -> dict:
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"Couldn't borrow book: {resp.json()}, you may need to verify your card in the app."
            )
        return resp.json()
    ...
    def hold_book(self, title_id: str, card_id: str) -> dict:
        ...
        j = {"days_to_suspend": 0, "email_address": ""}
        url = f"https://sentry-read.svc.overdrive.com/card/{card_id}/hold/{title_id}"
        resp = self.http_session.post(url, json=j, timeout=self.timeout)
        if resp.status_code != 200:
            raise RuntimeError(
                f"Couldn't hold book: {resp.json()}, you may need to verify your card in the app."
            )
        return resp.json()
    ...
    def cancel_hold(self, title_id: str, card_id: str = None):
        ...
        if not card_id:
            raise RuntimeError(
                "Couldn't find cardId on hold or couldn't find hold at all, can't cancel it."
            )
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"Couldn't cancel hold on book: {resp.json()}, you may need to verify your card in the app."
            )
    ...
    def hold_book_on_library_with_shortest_wait_time(self, title_id: str) -> dict:
        ...
        for card in sync["cards"]:
            if is_book_available(card["advantageKey"], title_id, timeout=self.timeout):
                ...
                return {}
            a = requests.get(
                f"https://thunder.api.overdrive.com/v2/libraries/{card['advantageKey']}/media/{title_id}/availability",
                timeout=self.timeout,
            ).json()
            a["cardId"] = card["cardId"]  # Add back the cardId so we can find it later
            a["library"] = card[
                "advantageKey"
            ]  # Add back library so we can find it later
        ...
        try:
            shortest = next(
                iter(
                    sorted(
                        (a for a in availabilities if "estimatedWaitDays" in a),
                        key=lambda i: i["estimatedWaitDays"],
                    )
                ),
                None,
            )
            if shortest:
                print(
                    f"Found shortest wait at {shortest['library']}, creating hold there."
                )
    ...
    def borrow_book_on_any_logged_in_library(
        self, title_id: str, days: int = 21
    ) -> dict:
        for card in self.get_sync()["cards"]:
            if int(card["counts"]["loan"]) >= int(card["limits"]["loan"]):
                print(
                    f"Card {card['cardId']} at {card['advantageKey']} is at its limit, skipping."
                )
            elif is_book_available(
                card["advantageKey"], title_id, timeout=self.timeout
            ):
    ...
    def return_book(self, title_id: str, card_id: str = None):
        ...
        if not card_id:
            raise RuntimeError(
                "Couldn't find cardId on loan or couldn't find loan at all, can't return it."
            )
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"Couldn't return book: {resp.json()}, you may need to verify your card in the app."
            )

    def get_sync(self) -> dict:
        return self.http_session.get(
            "https://sentry-read.svc.overdrive.com/chip/sync", timeout=self.timeout
        ).json()
    ...
    def get_chip(self) -> dict:
        ...
        self.http_session.headers.update(
            {"Authorization": f'Bearer {response["identity"]}'}
        )
    ...
    def get_loan(self, title_id: str) -> dict:
        return next(
            (loan for loan in self.get_sync()["loans"] if loan["id"] == title_id), {}
        )
    ...
    def get_hold(self, title_id: str) -> dict:
        return next(
            (hold for hold in self.get_sync()["holds"] if hold["id"] == title_id), {}
        )
    ...
    def open_audiobook(self, card_id: str, title_id: str) -> dict:
        ...
        return {
            ...
            "openbook": self.http_session.get(
                openbook_url, timeout=self.timeout
            ).json(),
            "media_info": get_media_info(title_id, timeout=self.timeout),
        }
    ...
    def search_for_book_in_logged_in_libraries(self, query: str) -> list:
        ...
        return requests.get(
            f"https://thunder.api.overdrive.com/v2/media/search?libraryKey={'libraryKey='.join([card['advantageKey'] + '&' for card in self.get_sync()['cards']])}query={query}",
            timeout=self.timeout,
        ).json()
    ...
    def search_for_audiobook_in_logged_in_libraries(self, query: str) -> list:
        return [
            h
            for h in self.search_for_book_in_logged_in_libraries(query)
            if h["type"]["id"] == "audiobook"
        ]
    ...
    def download_audiobook_mp3(
        ...
        should_replace_space=False,
        should_create_opf=False,
    ):
        ...
        download_urls = [
            audiobook_info["audiobook_urls"]["urls"]["web"] + s["path"]
            for s in audiobook_info["openbook"]["spine"]
        ]
            ...
            if self.should_download(loan["id"], filename):
                ...
                if should_embed_metadata:
                    if filename in tocout:
                        ...
                        embed_tag_data(
                            os.path.join(final_path, filename),
                            "<Markers><Marker><Name>(continued)</Name><Time>0:00.000</Time></Marker></Markers>",
                            audiobook_info,
                            cover_file_path,
                        )
                        print(
                            "no toc to embed, generated (continued) chapter marker, and embedded it."
                        )
                ...
                self.add_to_archive(
                    ...
                    if "firstCreatorName" in loan
                    else get_authors(loan["id"]),
                    loan["title"] if "title" in loan else None,
                )
    ...
    def download_loan(
        ...
        if self.archive_path:
            ...
            if self.is_downloaded(loan["id"]):
                print(
                    f"Book has already been downloaded and stored in archive: {loan['id']}"
                )
                return
        ...
        if format_is_available:
            ...
            if format_id == "audiobook-mp3":
                if should_get_odm:
                    if format_string is not None:
                        ...
                        download_path = get_download_path(
                            media_info, should_replace_space=should_replace_space
                        )
                    ...
                    if "fulfill" in fulfill:
                        if should_download:
                            ...
                            if self.should_download(loan["id"], loan["id"] + ".odm"):
                                with open(
                                    os.path.join(final_path, loan["id"] + ".odm"), "wb"
                                ) as w:
                                    w.write(
                                        self.http_session.get(
                                            fulfill_url, timeout=self.timeout
                                        ).content
                                    )
                                    print(f"Downloaded odm file to {w.name}.")
                                    self.add_to_archive(
                                        loan["id"],
                                        os.path.basename(w.name),
                                        loan["firstCreatorName"]
                                        if "firstCreatorName" in loan
                                        else get_authors(loan["id"]),
                                        loan["title"] if "title" in loan else None,
                        ...
                        raise RuntimeError(
                            f"Something went wrong when downloading odm: {fulfill}"
                        )
                else:
                    self.download_audiobook_mp3(
                        loan,
                        output_path,
                        should_save_info=should_save_info,
                ...
                if "fulfill" in fulfill:
                    ...
                    if format_id == "audiobook-overdrive":
                        ...
                        print(
                            "Will download acsm file, use a tool like Knock (https://github.com/agschaid/knock) to get your book."
                        )
                        if should_download:
                            if self.should_download(
                                ...
                                os.path.basename(
                                    os.path.join(
                                        final_path, get_filename_from_url(fulfill_url)
                                    )
                                ),
                            ):
                                with open(
                        ...
                        if should_download:
                            ...
                            if self.should_download(
                                loan["id"], os.path.basename(filename)
                            ):
            ...
            raise RuntimeError(
                f"Format {format_id} not available for title {loan['id']}. Available formats: {str([f['id'] for f in loan['formats']])}."
            )
    ...
    def add_to_archive(
        self, title_id: str, filename: str, author: str = None, title: str = None
    ):
        if self.archive_path:
            self.load_archive()
            ...
            if title_id not in self.archive:
                ...
                if self.archive[title_id]["Finished"]:
                    ...
                    if filename in self.archive[title_id]["Parts"]:
                        ...
                        print(
                            f"Title: {title_id} was not finished, {filename} was missing, downloading."
                        )
                        return True
...
def main():
    ...
    parser.add_argument(
        ...
        default=os.getenv("ID", "./config/id.json"),
        metavar="path",
    )
    parser.add_argument(
        "-c",
        "--code",
        help="Login with code.",
        type=int,
        metavar="12345678",
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output dir, will output to current dir if omitted.",
    ...
    parser.add_argument(
        "-s",
        "--search",
        help="Search for book in your libraries.",
        metavar='"search query"',
    )
    parser.add_argument(
        "-sa",
        "--search-audiobook",
        help="Search for audiobook in your libraries.",
        metavar='"search query"',
    )
    parser.add_argument(
        "-se",
        "--search-ebook",
        help="Search for ebook in your libraries.",
        metavar='"search query"',
    )
    parser.add_argument(
        "-ls", "--list-loans", help="List your current loans.", action="store_true"
    )
    parser.add_argument(
        "-lsc", "--list-cards", help="List your current cards.", action="store_true"
    )
    parser.add_argument(
        "-lsh", "--list-holds", help="List your current holds.", action="store_true"
    )
    parser.add_argument(
        "-b",
        "--borrow-book",
        help="Borrow book from the first library where it's available.",
        metavar="id",
    )
    parser.add_argument(
        "-r",
    ...
    parser.add_argument(
        "-ho",
        "--hold-book",
        help="Hold book from the library with the shortest wait.",
        metavar="id",
    )
    parser.add_argument(
        "-ch",
    ...
    parser.add_argument(
        "-f",
        "--format",
        help="Which format to download with -dl.",
        type=str,
        metavar="id",
    ...
    parser.add_argument(
        "-dla",
        "--download-all",
        help="Download all loans with the specified format. Does not consider -f.",
        metavar="format",
        default=os.getenv("DOWNLOAD_ALL"),
    )
    parser.add_argument(
        "-odm",
        help="Download the ODM instead of directly downloading mp3's for 'audiobook-mp3'.",
        action="store_true",
    )
    parser.add_argument(
        "-si",
        "--save-info",
        help="Save information about downloaded book.",
        action="store_true",
        default=os.getenv("SAVE_INFO"),
    )
    parser.add_argument(
        "-i", "--info", help="Print media info (JSON).", type=str, metavar="id"
    )
    parser.add_argument(
        "-a",
        "--archive",
        help="Path to archive file. The archive keeps track of what is already downloaded. Defaults to ./config/archive.json",
        default=os.getenv("ARCHIVE", "./config/archive.json"),
        type=str,
        metavar="path",
    )
    parser.add_argument(
        "-j",
        "--json",
        help="Output verbose JSON instead of tables.",
        action="store_true",
    )
    parser.add_argument(
        "-e",
        "--embed-metadata",
        help="Embeds metadata in MP3 files, including chapter markers.",
        action="store_true",
        default=os.getenv("EMBED_METADATA"),
    )
    parser.add_argument(
        "-opf",
        "--create-opf",
        help="Create an OPF file with metadata when downloading a book.",
        action="store_true",
        default=os.getenv("CREATE_OPF"),
    )
    parser.add_argument(
        "-dlo",
        "--download-opf",
        help="Generate an OPF file by title id.",
        type=str,
        metavar="id",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-rs",
        "--replace-space",
        help="Replace spaces in folder path with underscores.",
        action="store_true",
        default=os.getenv("REPLACE_SPACE"),
    )
    parser.add_argument(
        "-t",
        "--timeout",
        help="Download timeout interval (seconds).",
        type=int,
    ...
    def create_table(media_infos: list, narrators=True):
        ...
        for m in media_infos:
            row = {
                ...
                "Libraries": "\n".join(
                    lib + ": available"
                    if m["siteAvailabilities"][lib]["isAvailable"]
                    else lib + ": unavailable"
                    for lib in m["siteAvailabilities"].keys()
                ),
    ...
    for arg in sys.argv:
        if arg in ["-ls", "--list-loans"]:
                    ...
                    t.append(
                        {
                            ...
                            "Title": h["title"],
                            "Narrators": "\n".join(get_narrators(mi).split(" & ")),
                            "Estimated Wait": f"{h['estimatedWaitDays']} Days\nNumber {h['holdListPosition']} in line"
                            if h.keys() >= {"estimatedWaitDays", "holdListPosition"}
                            else "",
                        }
                    )
                print(tabulate(t, headers="keys", tablefmt="grid"))
            ...
            if args.json:
                ...
                print("Cards:")
                for c in s["cards"]:
                    t.append({"Id": c["cardId"], "Library": c["advantageKey"]})
                print(tabulate(t, headers="keys", tablefmt="grid"))

        elif arg in ["-dl", "--download"]:
            print("Downloading", sys.argv[arg_pos + 1])
            L.download_loan(
                L.get_loan(sys.argv[arg_pos + 1]),
                args.format,
                args.output,
                args.save_info,
            ...
            if args.output_format_string:
                ...
                dl_path = get_download_path(
                    media_info, should_replace_space=args.replace_space
                )
            ...
            if args.json:
                ...
                print(
                    tabulate(
                        create_table(hits, narrators=False),
                        headers="keys",
                        tablefmt="grid",
                    )
                )
